<?php
// Form field, basic supports both the following plugins
...
if ( $return_format == 'id' ) {
	echo $this->indent . htmlspecialchars("<?php \$".$this->var_name."_id = " . $this->get_field_method . "( '" . $this->name ."'". $this->location . " ); ?>")."\n";
}

// If return format is Object 
if ( $return_format == 'post_object' ) {
	echo $this->indent . htmlspecialchars("<?php \$".$this->var_name."_object = " . $this->get_field_method . "( '" . $this->name ."'". $this->location . " ); ?>")."\n";
}
